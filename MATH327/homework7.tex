...
\begin{document}
    ...
    Assume a normal distribution.\\\newline
        ...
        $P(\bar{X} < 0.475) = P(T < -0.39) =$ 0.34. I would not agree with the claim.}\\\newline
    ...
    ity of the two population variances.\\\newline
        \textbf{Variance of the first group is 10.441 and the variance of the second
        group is 1.846. $f$ is then 5.66. This value is between $f_{0.05}$ and $f_{0.01}$,
        so $P(F>5.66)$ is small and therefore the variances of the first group and the
        second group are not equal.}\\\newline
...
\edef\numrows{\pgfplotsretval}
        ...
        \begin{tikzpicture}[
            ...
            \begin{axis}
                \addplot [only marks, mark=*] table [x index=0, y index=0, x expr=normq(\coordindex/\numrows)] {\sorted};
            \end{axis}
        \end{tikzpicture}\\\newline
    ...
    and $\sigma_2^2 = 15$, respectively, find $P(\sigma^2_1 / \sigma_2^2 > 1.26)$.\\\newline
        \textbf{$P(s^2_1 / s_2^2 > 1.26) = P((s^2_1 / \sigma^2_1) / (s^2_2 / \sigma^2_2) > (15 \cdot 1.26)/10) = P(F>1.89)$ Then $F$ values $v_1$ is 24 and $v_2$ is 30 and the probability is 0.05.}
\end{document}